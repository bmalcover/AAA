...
Per avaluar les capacitats d'un algoritme d'aprenentatge automàtic, cal dissenyar una mesura quantitativa del seu rendiment, _P_, específica per a la tasca que duu a terme el sistema. En l'aprenentatge automàtic, i en general en els mètodes d'optimització, aquesta mesura es coneix com a funció objectiu. Generalment es defineix de manera que com més baix és l'error del model, millor és el resultat, i per això rep també el nom de funció de pèrdua (_loss function_): en lloc de mesurar si el model ho fa bé, es mesura fins a quin punt ho fa malament.

Com que els models d'aprenentatge automàtic són models matemàtics, les funcions de pèrdua han d'estar definides de manera formal. La forma concreta d'aquesta funció depèn del tipus de problema. En els problemes de regressió, on l'objectiu és predir valors numèrics continus, la funció de pèrdua més habitual és l'error quadràtic, és a dir, el quadrat de la diferència entre la predicció i el valor desitjat. En els problemes de classificació, en canvi, l'objectiu més comú és minimitzar la taxa d'error: la fracció de mostres en què la predicció del model no coincideix amb l'etiqueta real.
